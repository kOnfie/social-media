...
const buttonVariants = cva("rounded-[10px]", {
  variants: {
    variant: {
      ...
      ghost:
        "text-primary bg-green-light hover:bg-green-light-hover hover:primary-hover border border-primary border-solid",
    },
    size: {
      // default: "px-4 py-2",
    },
...
function CustomButton({
  ...
  return (
    <Comp
      data-slot="button"
      className={cn("text-[14px] cursor-pointer transition-all", buttonVariants({ variant, size, className }))}
      {...props}
    />
  );
}
